...
// ===== CONFIGURAÇÕES =====
const DIAS_PADRAO = 2;
const MIN_PALAVRAS_CHAVE = 1; // Reduzido para 1 para capturar mais notícias
...
// ===== FUNÇÃO: DETECTAR CATEGORIA (USANDO O FIRESTORE) =====
function detectarCategoria(titulo, resumo, categoriaFonte, categoriasFirestore) {
  const texto = (titulo + ' ' + resumo).toLowerCase();

  // Se a fonte já tem uma categoria definida, usa ela
  if (categoriaFonte && categoriaFonte !== 'geral') {
    return categoriaFonte;
  }

  // ===== PRIORIDADE: SEGURANÇA =====
  const seguranca = ['roubo', 'assalto', 'carga', 'criminalidade', 'violência', 
                     'tiroteio', 'confronto', 'operação policial', 'prf', 'blitz',
                     'bandido', 'traficante', 'apreensão', 'flagrante', 'investigação',
                     'vigilância', 'segurança pública', 'carga roubada', 'incêndio', 
                     'fogo', 'queimada', 'desaparece', 'desaparecido', 'espancado',
                     'agressão', 'morte', 'homicídio'];
  ...
  const greve = ['greve', 'paralisação', 'caminhoneiro', 'bloqueio', 'protesto',
                 'piquete', 'manifestação', 'travamento'];
  ...
  // ===== 3º PRIORIDADE: CLIMA (USA AS PALAVRAS DO FIRESTORE) =====
  ...
  const acidente = ['acidente', 'colisão', 'capotamento', 'engavetamento',
  ...
  const transito = ['interdição', 'rodovia', 'br-', 'trânsito',
                    'congestionamento', 'desvio', 'obras'];
...
async function coletarNoticias() {
  ...
  console.log(`📋 Mínimo de ${MIN_PALAVRAS_CHAVE} palavra-chave obrigatória`);
  ...
  for (const fonte of FONTES) {
    try {
      ...
      for (const item of feed.items.slice(0, 8)) {
        ...
        // ===== DETECTA CATEGORIA (USANDO FIRESTORE) =====
